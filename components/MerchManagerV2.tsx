...
export default function MerchManagerV2() {
  ...
  async function generateListing(nextInput: ProductInput, images = sourceImages, autoUgc = false) {
    ...
    try {
      ...
      if (autoUgc && images.length) void generateUgcPack(generated, images, ugcCount);
      else setStatus("Product information and AI copy are ready.");
    } catch (e) {
  ...
  async function researchProduct(target: string) {
    ...
    try {
      ...
      const nextInput = d.input as ProductInput;
      const images = (d.scraped?.images || []) as string[];
      setInput(nextInput);
      setSourceImages(images);
      if (!images.length) setStatus("Product information was found, but the source site did not expose its product gallery.");
      await generateListing(nextInput, images, true);
    } catch (e) {
      setStatus(e instanceof Error ? e.message : "Could not read that product page.");
    } finally {
  ...
  useEffect(() => {
    ...
    if (!/^https?:\/\//i.test(trimmed)) return;
    const t = setTimeout(() => void researchProduct(trimmed), 700);
  ...
  async function hostGeneratedImage(imageUrl: string, index: number) {
    ...
    try {
      const r = await fetch("/api/shopify/upload-image", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dataUrl: imageUrl, filename: `ugc-${Date.now()}-${index + 1}.png` }) });
      const d = await r.json();
      return r.ok && d.imageUrl ? String(d.imageUrl) : imageUrl;
    } catch { return imageUrl; }
  }

  async function generateUgcPack(product = result, images = sourceImages, count = ugcCount) {
    if (!product || !images.length || ugcBusy) return;
    setUgcBusy(true); setUgcImages([]); setSelectedImage(0);
    try {
      for (let i = 0; i < count; i++) {
        setStatus(`Generating UGC image ${i + 1} of ${count} from the original product photography…`);
        const sourceImageUrl = images[i % Math.min(images.length, 4)];
        const prompt = `${UGC_PROMPTS[i % UGC_PROMPTS.length]} Product: ${product.title}. Product facts: ${htmlText(product.descriptionHtml).slice(0, 900)}`;
        ...
        const d = await r.json();
        if (!r.ok) throw new Error(d.error || `UGC image ${i + 1} failed.`);
        const hosted = await hostGeneratedImage(d.imageUrl, i);
        setUgcImages(prev => [...prev, hosted]);
      }
      setStatus(`${count} UGC images are ready and attached to this product package.`);
    } catch (e) {
      setStatus(e instanceof Error ? e.message : "UGC generation failed.");
    } finally { setUgcBusy(false); }
  }

  async function publish() {
    if (!result) return;
    setPublishing(true); setStatus("Publishing Shopify draft…");
    try {
  ...
  async function postPinterest() {
    ...
    setStatus("Posting pin…");
    try {
      const r = await fetch("/api/social/pinterest", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: pinterestTitle, description: pinterestDescription, imageUrl: previewImage, link: result.ctaButtonUrl || input.url }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "Pinterest posting failed.");
      setStatus("Posted to Pinterest.");
    } catch (e) { setStatus(e instanceof Error ? e.message : "Pinterest posting failed."); }
  }

  return <div className={styles.wrap}>
    <header className={styles.header}>
      <div className={styles.kicker}>FCP MERCH MANAGER</div>
      <h1>Paste a product. Build the real listing.</h1>
      <p>Pull the original product data, rewrite the copy, create UGC assets, preview the exact product page, and publish.</p>
    </header>

    <section className={styles.card}>
      <label className={styles.label}>Product URL</label>
      <div className={styles.urlRow}><input value={url} onChange={e => setUrl(e.target.value)} placeholder="Paste Amazon, Mavely, Walmart, supplier or product URL"/><button onClick={() => researchProduct(url)} disabled={researching}>{researching ? "Reading…" : "Pull product"}</button></div>
    ...
    {(input.name || sourceImages.length > 0) && <section className={styles.card}>
      ...
      <div className={styles.formGrid}>
        <label><span>Product title</span><input value={input.name} onChange={e => setInput(v => ({ ...v, name: e.target.value }))}/></label>
        <label><span>Price</span><input type="number" step="0.01" value={input.price || ""} onChange={e => setInput(v => ({ ...v, price: Number(e.target.value) }))} placeholder="Current price"/></label>
        <label><span>Category</span><input value={input.category} onChange={e => setInput(v => ({ ...v, category: e.target.value }))}/></label>
        <label><span>Merchant</span><input value={input.merchant} onChange={e => setInput(v => ({ ...v, merchant: e.target.value }))}/></label>
      </div>
      ...
      <details className={styles.sourceCopy}><summary>Original product description</summary><p>{input.sourceDescription}</p></details>
      <button className={styles.primary} onClick={() => generateListing(input)} disabled={generating}>{generating ? "Writing…" : "Regenerate AI listing"}</button>
    </section>}
    ...
    {result && <>
      ...
      <section className={styles.card}>
        ...
        <div className={styles.productPage}>
          <div className={styles.gallery}>
            <div className={styles.mainImage}>{previewImage ? <img src={previewImage} alt={result.title}/> : <div>No product image</div>}</div>
            <div className={styles.thumbRail}>{allImages.map((src, i) => <button key={`${src}-${i}`} className={i === selectedImage ? styles.activeThumb : ""} onClick={() => setSelectedImage(i)}><img src={src} alt=""/></button>)}</div>
          </div>
      ...
      <section className={styles.card}>
        <div className={styles.kicker}>SOCIAL CREATIVE</div><h2>Ready-to-post creative</h2>
        <div className={styles.socialGrid}>
          <div>{previewImage && <img className={styles.socialImage} src={previewImage} alt="Selected social creative"/>}<div className={styles.miniStrip}>{ugcImages.map((src, i) => <button key={`${src}-${i}`} onClick={() => setSelectedImage(i)}><img src={src} alt=""/></button>)}</div></div>
          <label className={styles.caption}><span>AI caption</span><textarea rows={10} value={socialCaption} onChange={e => setSocialCaption(e.target.value)}/><button onClick={() => navigator.clipboard.writeText(socialCaption)}>Copy caption</button></label>
        </div>
      </section>

      <section className={styles.card}>
        <div className={styles.kicker}>PINTEREST</div><h2>Create a Pin</h2>
        <div className={styles.pinterestGrid}>{previewImage && <img src={previewImage} alt="Pinterest creative"/>}<div className={styles.pinFields}><label><span>Pin title</span><input value={pinterestTitle} onChange={e => setPinterestTitle(e.target.value)}/></label><label><span>Description</span><textarea rows={6} value={pinterestDescription} onChange={e => setPinterestDescription(e.target.value)}/></label><button className={styles.primary} onClick={postPinterest}>Post to Pinterest</button></div></div>
      </section>
    </>}
  </div>;
}
