...
function amazonPrice(html: string): number | null {
  ...
  for (const re of patterns) {
    const m = scoped.match(re); if (!m) continue;
...
function amazonBullets(html: string): string | null {
  ...
  const bullets = Array.from(block.matchAll(/<span[^>]*class=["'][^"']*a-list-item[^"']*["'][^>]*>([\s\S]*?)<\/span>/gi))
    .map(m => text(m[1])).map(v => v.replace(/^About this item\s*/i, "").trim())
...
function amazonGallery(html: string): string[] {
  ...
  if (colorBlock) for (const m of colorBlock.matchAll(/["'](?:hiRes|large)["']\s*:\s*["'](https?:[^"']+)["']/gi)) candidates.push(normalizeAmazonImage(m[1]));
  if (!candidates.length) {
    const landing = html.match(/id=["']landingImage["'][^>]*data-a-dynamic-image=["']([^"']+)["']/i)?.[1];
    if (landing) try { candidates.push(...Object.keys(JSON.parse(decode(landing))).map(normalizeAmazonImage)); } catch { /* ignore */ }
  }
...
export async function enforceSourceTruth(url: string, base: ScrapedProduct & { blocked?: boolean }): Promise<ScrapedProduct & { blocked?: boolean }> {
  ...
  try {
    const response = await fetch(url, { redirect: "follow", cache: "no-store", signal: controller.signal, headers: {
      "User-Agent": "Mozilla/5.0 (compatible; FortCrazypantsBot/1.0; +https://fortcrazypants.com)", "Accept": "text/html,application/xhtml+xml"
    }});
    const html = await response.text();
    const blocked = /Type the characters you see|Enter the characters you see below/i.test(html);
    if (blocked) return { title: base.title, price: base.price, description: null, images: [], blocked: true };
    ...
    // CRITICAL: for Amazon, never fall back to generic OG description/images. Those can come
    // from ads, recommendations or a different product and caused cross-product copy/images.
    return { title: title || base.title, price: price || base.price, description, images, blocked: false };
  } catch {
    return { title: base.title, price: base.price, description: null, images: [], blocked: true };
